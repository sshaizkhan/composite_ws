...
LagrangeMultiplierSolver::LagrangeMultiplierSolver(const SparseMatrix * A, const SparseMatrix * J, const SparseMatrix * B, int numFixedDOFs_, const int * fixedDOFs_, int numThreads_, int updatable, int addDirichlet) : numFixedDOFs(numFixedDOFs_), numThreads(numThreads_)
{
  ...
  systemMatrix->RemoveRowsColumns(numFixedDOFs, fixedDOFs.data());
  ...
  if (addDirichlet)
  {
    ...
    ADirichlet->RemoveRows(numFixedDOFs, fixedDOFs.data());

    vector<int> remainedDOFs(mFree);
    ConstrainedDOFs::FindFreeDOFs(m, &remainedDOFs[0], numFixedDOFs, fixedDOFs.data());

    ADirichlet->RemoveColumns(remainedDOFs.size(), remainedDOFs.data());

    assert(ADirichlet->GetNumColumns() <= numFixedDOFs);
    if (updatable)
      ADirichlet->BuildSuperMatrixIndices(numFixedDOFs, fixedDOFs.data(), remainedDOFs.size(), remainedDOFs.data(), A);
  ...
  if (updatable)
  {
    ...
    Acons->RemoveRowsColumns(numFixedDOFs, fixedDOFs.data());
    Acons->BuildSuperMatrixIndices(numFixedDOFs, fixedDOFs.data(), A);

    if (c > 0)
    {
      Jcons = new SparseMatrix(*J);
      Jcons->RemoveColumns(numFixedDOFs, fixedDOFs.data());
      Jcons->BuildSuperMatrixIndices(0, NULL, numFixedDOFs, fixedDOFs.data(), J);
...
MKL_INT LagrangeMultiplierSolver::SolveLinearSystem(double * x, const double * rhs)
{
  ...
  ConstrainedDOFs::RemoveDOFs(m+c, &rhsConstrained[0], rhs, numFixedDOFs, fixedDOFs.data());

  AddDirichletOnRhs(x);

  MKL_INT code =  pardisoSolver->SolveLinearSystem(&xConstrained[0], &rhsConstrained[0]);
  ConstrainedDOFs::InsertDOFs(m+c, &xConstrained[0], x, numFixedDOFs, fixedDOFs.data());
...
{
  ConstrainedDOFs::RemoveDOFs(m+c, &rhsConstrained[0], rhs, numFixedDOFs, fixedDOFs.data());
  ConstrainedDOFs::RemoveDOFs(m+c, &xConstrained[0], x, numFixedDOFs, fixedDOFs.data());
  AddDirichletOnRhs(x);
