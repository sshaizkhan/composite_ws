...
std::vector<int> c_pts;
std::vector<int> fixedpts;
std::vector<int> sc_list;
std::vector<std::vector<double>> meshpts;
// std::vector<Vec3d> meshpts;
...
/////////////////////////////////////////////////
int constrain_test=0;//test insertDOF()
int new_fixpnt[2]={3,4};
//////////////////////////////////////////////////
SCn::SheetCollision *SC;
...
void displayFunction()
{
  ...
  { 
    ...
    if (renderFixedVertices)
    {
      for(int i=0; i<fixedpts.size(); i++)
      {
        ...
        sceneObjDeform->GetSingleVertexPositionFromBuffer(fixedpts[i],
  ...
  if (pulledVertex != -1)
  {
    ...
    //dynamic fix test
    // printf("Before set U:\n U: (%G, %G, %G)\n", u[3*pulledVertex + 0], u[3*pulledVertex + 1], u[3*pulledVertex + 2]);
    // integratorBase->SetQ(pulledVertex, 0);
    // integratorBase->SetqState(u);
    // sceneObjDeform->SetVertexDeformations(u);
    // printf("After set U:\n U: (%G, %G, %G)\n", u[3*pulledVertex + 0], u[3*pulledVertex + 1], u[3*pulledVertex + 2]);
  }
...
void idleFunction(void)
{
  ...
  if (runSimulation == 1) // if unpaused
  {
    ...
    {
      for(int j=0; j<3; j++)
       temppts.push_back(sceneObjDeform->GetSingleVertexPositionFromBuffer(i)[j]);
     meshpts.push_back(temppts);
    temppts.clear();
    }
    SC->idxQuery(meshpts, sc_list);
    meshpts.clear();
    ///////////////////////////////////////
    //debug purpose/////////
    printf("number of contacts %li\n",sc_list.size());
    if (sc_list.size() != 0)
    {
      printf("index of points: ");
  ...
  if(sc_list.size()!=0)
  {
    ...
    constrain_test=0;
    // int num_newpts=sizeof(new_fixpnt)/sizeof(new_fixpnt[0]);
    int num_newpts=sc_list.size();
    // std::cout<<"pts: "<< num_newpts<<std::endl;
    int *constrains=NULL;
    constrains = (int*) malloc (sizeof(int) * 3*num_newpts);
    for(int i=0; i< num_newpts; i++)
    {
      constrains[3*i+0]=sc_list[i]*3+0;
      constrains[3*i+1]=sc_list[i]*3+1;
      constrains[3*i+2]=sc_list[i]*3+2;
      fixedpts.push_back(sc_list[i]);
      c_pts.push_back(constrains[3*i+0]);
      c_pts.push_back(constrains[3*i+1]);
      c_pts.push_back(constrains[3*i+2]);
    }
    std::sort(fixedpts.begin(),fixedpts.end());
    std::sort(c_pts.begin(),c_pts.end());
    fixedpts.resize(std::distance(fixedpts.begin(),std::unique(fixedpts.begin(), fixedpts.end())));
    c_pts.resize(std::distance(c_pts.begin(),std::unique(c_pts.begin(), c_pts.end())));
    // std::cout<< "new first : "<< c_pts[0]<<std::endl;
    std::cout<<"fixed size: "<< c_pts.size()<<std::endl;
      // memcpy(u, integratorBase->Getq(), sizeof(double) * 3 * n);
    // for(int i=0; i<n; i++)
    //     std::cout<<"vertex "<< i <<":"<<sceneObjDeform->GetSingleVertexPositionFromBuffer(i)<<std::endl;
    ...
    implicitBackwardEulerSparse = new ImplicitBackwardEulerSparse(3 * clothBW->GetNumVertices(), timeStep, massMatrix, forceModel, c_pts.size(), &c_pts[0], dampingMass, dampingStiffness, 1, 1E-5, numSolverThreads);
    std::cout<< "new class done"<<std::endl;
  ...
  // if(runSimulation==0)
  // {
  //   // std::cout<< std::endl<<"paused u:"<< std::endl;
  //   // for(int i=0; i<3*n; i++)
  //   //   std::cout<<u[i]<<std::endl;
  //   // for(int i=0; i<n; i++)
  //   //     std::cout<<"vertex "<< i <<":"<<sceneObjDeform->GetSingleVertexPositionFromBuffer(i)<<std::endl;
...
void initScene()
{
  ...
  */
  ...
  for (int i=0; i<numFixedVertices; i++)
  {
    ...
    fixedpts.push_back(fixedVertices[i]);
    c_pts.push_back(constrainedDOFs[3*i+0]);
    c_pts.push_back(constrainedDOFs[3*i+1]);
    c_pts.push_back(constrainedDOFs[3*i+2]);
  ...
  if(!meshpts.empty())
   ...
   // std::cout << "numCons:" <<numConstrainedDOFs<<std::endl;
  //  std::cout << "old first:" <<c_pts[0]<<std::endl;
  // std::cout << "old_size:" <<c_pts.size()<<std::endl;
  // now create renderer
  if (sceneObjDeform != NULL)
    delete sceneObjDeform;
  ...
  if (implicitBackwardEulerSparse != NULL)
    delete implicitBackwardEulerSparse;
  implicitBackwardEulerSparse = new ImplicitBackwardEulerSparse(3 * numVertices, timeStep, massMatrix, forceModel, c_pts.size(), &c_pts[0], dampingMass, dampingStiffness, 1, 1E-5, numSolverThreads);
  implicitNewmarkSparse = implicitBackwardEulerSparse;
  integratorBase = implicitNewmarkSparse;
  ...
  if (strcmp(extraSceneGeometryFilename,"__none") != 0)
  {
    ...
    std::cout << "extraGeom done" << std::endl;
    // extraSceneGeometry->EnableCustomColor();
  }
  else
  {
    extraSceneGeometry = NULL;
    std::cout << "NO extraGeom " << std::endl;
  }
    // std::cout <<"Meshname: "<< objMeshname<<std::endl;
  ///initialize sheetcollision obj
  SC = new SCn::SheetCollision();
  SC->init(objMeshname, extraSceneGeometryFilename);
