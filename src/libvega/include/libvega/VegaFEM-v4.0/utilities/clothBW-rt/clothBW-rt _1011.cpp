...
void displayFunction()
{
  ...
  //   {
  //   fixCoords[0] = restPos[fixpnt[i]][0] + u[3*fixpnt[i] + 0];
  //   fixCoords[1] = restPos[fixpnt[i]][1] + u[3*fixpnt[i] + 1];
  //   fixCoords[2] = restPos[fixpnt[i]][2] + u[3*fixpnt[i] + 2];
  //   // std::cout << *restPos;
  //   // printf("Vertex: (%G, %G, %G)\n", vertexCoords[0], vertexCoords[1], vertexCoords[2]);
    
  //   glVertex3f(fixCoords[0], fixCoords[1], fixCoords[2]);
  //   }
...
void idleFunction(void)
{
  ...
  if (runSimulation == 1) // if unpaused
  {
    ...
    if(constrain_test==1)
    {
      ...
      {
        constrains[3*i+0]=new_fixpnt[i]*3+0;
        constrains[3*i+1]=new_fixpnt[i]*3+1;
        constrains[3*i+2]=new_fixpnt[i]*3+2;
        fixedpts.push_back(new_fixpnt[i]);
      ...
      std::cout<<"new size: "<< c_pts.size()<<std::endl;
      // memcpy(u, integratorBase->Getq(), sizeof(double) * 3 * n);
    for(int i=0; i<n; i++)
